...
public class UserEndpoint {
	...
	public Response findById(@PathParam("id") Long id,@Context Request req) {
		...
		if (entity == null) {
			return Response.status(Status.NOT_FOUND).build();//tu nie daje mozliwosci buforowania w cache na 1 dzien bo moze byc dodany uzytkownik w kazdej chwili.
		}
		ResponseBuilder rb=null;
		CacheControl cc= new CacheControl();
		cc.setPrivate(true);
		cc.setMaxAge(86400);//1 day max
		EntityTag etag=new EntityTag(Integer.toString(entity.hashCode()));
		rb=req.evaluatePreconditions(etag);
		if(rb!=null){
			return rb.cacheControl(cc).tag(etag).build();
		}
		rb=Response.ok(entity).cacheControl(cc).tag(etag);
		return rb.build();
	}
